...
num_cols_cluster_feat = ["authorFollowers", "account_age_days", "mentions_count",
                         "hashtags_count", "content_length"]  # <- sin toxicity_score

tmp = df[num_cols_cluster_feat + ["toxicity_score"]].copy()
tmp[num_cols_cluster_feat + ["toxicity_score"]] = tmp[num_cols_cluster_feat + ["toxicity_score"]].apply(pd.to_numeric, errors="coerce")

# Mantenemos filas completas para poder luego cruzar contra el target
tmp = tmp.dropna(subset=num_cols_cluster_feat + ["toxicity_score"]).reset_index(drop=True)

dfX = tmp[num_cols_cluster_feat]        # solo features para clusterizar
dfY = tmp["toxicity_score"]             # target guardado para análisis posterior
...
for k in k_range:
    ...
    pipe_km.fit(dfX)
    labels = pipe_km.named_steps["km"].labels_
    Xs = pipe_km.named_steps["scaler"].transform(dfX)   # silhouette en espacio escalado
    inertia = pipe_km.named_steps["km"].inertia_
...
print("Silhouette por k:", dict(zip(k_range, np.round(silhouette_scores, 4))))
print(f"Mejor k (Silhouette): {best['k']}  |  Silhouette={best['score']:.4f}")

# Gráficos: Codo e Silhouette
...
for i, v in enumerate(inertias):
    axes[0].text(list(k_range)[i], v, f"{v:.0f}", ha="center", va="bottom", fontsize=8)

axes[1].plot(list(k_range), silhouette_scores, "o-r")
axes[1].set_title("Silhouette vs k")
axes[1].set_xlabel("k"); axes[1].set_ylabel("Silhouette")
for i, v in enumerate(silhouette_scores):
    axes[1].text(list(k_range)[i], v+0.01, f"{v:.3f}", ha="center", va="bottom", fontsize=8)

plt.tight_layout(); plt.show()
```

# Visualización y relación con el target binario

```{python}
# Construimos df de visualización uniendo features, cluster y el target continuo
dfvis = dfX.copy()
dfvis["toxicity_score"] = dfY.values
dfvis["cluster"] = best["labels"]

# Distribución de clusters
counts = dfvis["cluster"].value_counts().sort_index()
for cl, cnt in counts.items():
    pct = cnt / len(dfvis) * 100
    print(f"Cluster {cl}: {cnt} registros ({pct:.1f}%)")

# Scatter 2D: una feature interpretable vs el target continuo (el target NO se usó para clusterizar)
for cl in sorted(dfvis["cluster"].unique()):
    part = dfvis[dfvis["cluster"] == cl]
    plt.scatter(part["content_length"], part["toxicity_score"], alpha=0.6, label=f"Cluster {cl}")
plt.legend()
plt.title(f"Clusters KMeans (k={best['k']})")
plt.xlabel("content_length"); plt.ylabel("toxicity_score")
plt.tight_layout(); plt.show()

# Cruce contra clase binaria (0.5)
y_bin_all = (dfvis["toxicity_score"] >= 0.5).astype(int)
ct = pd.crosstab(dfvis["cluster"], y_bin_all, normalize="index") * 100
print("\n% de tóxicos por cluster (umbral 0.5):")
print(ct.round(1))
```
# Comparación con Agglomerative + PCA

```{python}

Xs = best["model"].named_steps["scaler"].transform(dfX)

agg = AgglomerativeClustering(n_clusters=best["k"])
labels_agg = agg.fit_predict(Xs)
sil_agg = silhouette_score(Xs, labels_agg)

print(f"\nComparación de Silhouette:")
print(f"- KMeans (k={best['k']}): {best['score']:.4f}")
print(f"- Agglomerative (k={best['k']}): {sil_agg:.4f}")

# PCA 2D para visualización (sin incluir toxicity_score en el fit)
pca = PCA(n_components=2, random_state=42)
Xp = pca.fit_transform(Xs)

fig, axes = plt.subplots(1, 3, figsize=(18, 5))

axes[0].scatter(Xp[:, 0], Xp[:, 1], c=best["labels"], cmap="tab10", alpha=0.7, s=30)
axes[0].set_title(f"K-Means (k={best['k']})")
axes[0].set_xlabel(f"PC1 ({pca.explained_variance_ratio_[0]*100:.1f}%)")
axes[0].set_ylabel(f"PC2 ({pca.explained_variance_ratio_[1]*100:.1f}%)")

axes[1].scatter(Xp[:, 0], Xp[:, 1], c=labels_agg, cmap="tab10", alpha=0.7, s=30)
axes[1].set_title("Agglomerative")
axes[1].set_xlabel(f"PC1 ({pca.explained_variance_ratio_[0]*100:.1f}%)")
axes[1].set_ylabel(f"PC2 ({pca.explained_variance_ratio_[1]*100:.1f}%)")

toxbin = (dfY >= 0.5).astype(int).values
sc = axes[2].scatter(Xp[:, 0], Xp[:, 1], c=toxbin, cmap="RdYlBu", alpha=0.7, s=30)
axes[2].set_title("Clases verdaderas (Tóxico binario 0/1)")
axes[2].set_xlabel(f"PC1 ({pca.explained_variance_ratio_[0]*100:.1f}%)")
axes[2].set_ylabel(f"PC2 ({pca.explained_variance_ratio_[1]*100:.1f}%)")
cbar = plt.colorbar(sc, ax=axes[2]); cbar.set_label("Toxicidad (0=No, 1=Sí)")
...
- El dataset 1.5k tweets es utilizable, pero presenta nulos relevantes: toxicity_score (~10%) y hashtags (muy incompleto). No hay duplicados y algunas variables carecen de variación (p.ej., authorVerified casi constante), por lo que aportan poco al modelado.

- Las señales más útiles provienen del texto (content). Los metadatos numéricos (authorFollowers, mentions_count, content_length, etc.) muestran correlaciones débiles con la toxicidad; content_length es la más consistente (correlación positiva baja).

- El toxicity_score presenta sesgo a la derecha: la mayoría de los mensajes se concentra en valores bajos (poca toxicidad) y sólo una fracción pequeña alcanza niveles altos, a modo de valores extremos. En conjunto, la toxicidad media es reducida y la dispersión es amplia, sin un pico marcado alrededor de la media.
...
- Para clasificación se transformó toxicity_score en binario. El umbral 0.5 ofrece ~18% de positivos, que está cerca del objetivo práctico (~20%).

- Además, se realizó una exploración para determinar el mejorumbral no solo por balance, sino con una búsqueda de umbral precisión/recall en el set de prueba. En general, tuning de umbral > usar 0.5 fijo, sobre todo si interesa capturar más casos tóxicos.

# Rendimiento de modelos

- Clasificación (pipeline TF-IDF + Random Forest): resultados sólidos para un baseline con texto + metadatos. Con umbral 0.5 se observó AUC≈0.72, accuracy≈0.84, precision≈0.63 y recall≈0.25; al optimizar el umbral por F1, el recall mejora, pero a costa de algo de precisión, lo que puede ser preferible si el costo del falso negativo es alto. Analizando nuestra matriz de confusiòn el modelo es casi no “se equivoca levantando falsos tóxicos”, pero se le escapan muchos tóxicos.

- Regresión (predecir toxicity_score continuo): con LinearRegression se obtiene MAE≈0.17–0.22, RMSE≈0.22–0.28 y R²≈0.12. Las gráficas real vs. predicho y residuales vs. predicción, se evaluó MAE/RMSE/R², las opciones de regresión lineal y Ridge, en nuestra interpreteación Ridge da mejores resultados pero podría presentar un riesgo de infraajuste por lo que pensando den geralización optamos por proponer mantener la regresión lineal para esta aplicación.

- Clustering : con K-Means, segmentos razonables pero no perfectos, el resultado escogido luego de aplicar el método del codo y Silhouette fue 6 por Silhouette≈0.31 . Los clusters presentan prevalencias distintas de toxicidad al cruzarlos con el target binario 0.5, lo que sugiere segmentos útiles para perfilamiento (p.ej., textos más largos vs. más cortos, autores con mayor vs. menor actividad).La longitud del texto nos indica que hay un grupo de tuits cortos y nada tóxicos y otro de tuits cortos más tóxicos. Luego se realizón una vizualización utilizando el Agglomerative  y el PCA. Usando este análisis vemos un comportamiento similiar enter  Agglomerative y K-means (k=6), en donde mustran cierto solapamiento, de todas maneras se uso clases verdaderas en donde vemos una mejor gráfica pero aun no consolida fronteras definidas. 


# Patrones relevantes

Mayor longitud del contenido tiende a asociarse con mayor toxicidad, lo que nos muestra una relación débil pero consistente.

Fuentes/categorías por ejemplo source, isReply, muestran diferencias de media de toxicidad y de prevalencia de la clase tóxica; sirven para enriquecer el modelado.

Los clusters no replican exactamente las clases del target, pero sí muestran gradientes (algunos grupos concentran más casos "tóxicos").

## Pasos futuros / mejoras

# Datos y features

- Completar toxicity_score faltante para evitar perder tweets; estandarizar el parseo de hashtags y menciones (listas limpias, minúsculas) y considerar frecuencias o indicadores top-k.

- Fortalecer la limpieza del texto: en TF-IDF, si bien se realizó un trabajo que nos permitió continuar con los siguientes puntos se ve una oportunidad de mejora con un analísis más extenso, esto se le evidenció en la nube de palabras.

- Añadir rasgos temporales (hora/día) basado en experiencia del analista como podría ser fechas de debates, revelaciones de informaciones relevantes del ámbito político ecuatoriano y de estructura (nº emojis, nº signos de exclamación).

# Modelado

# Clasificación:

- Se utilizó en un inicio el modelo de regresión logistica, en un segundo paso se realizó el ejercicio con RandomForest, el cual automáticamente mostró mejoría del resultado por lo cual se optó por presenter el mismo en este proyecto, debido que mostró siginificativamente mejor performance no se hizo comparativa en este paso sino que se decidió utilizar únicamente RandomForest, por lo que podría haber oportunidad de mejora probando otros modelos.


# Regresión:

- Se evaluaron residuales para vizualizar el rendimiento del modelo en donde podemos ver una tendencia positiva en la comparativa de real vs. predicho, es decir que el modelo si está captando el comportamiento de los datos. Con la gráfica de residuales por otro lado tenemos un riesgo de infraajuste del modelo debido el R2 bajo. Por lo que sería interesante probar otros modelos para el análisis.   

# Clustering:

- Sería interesante añadir una señal semántica del texto al clustering usando TF-IDF para luego aplicar un TruncatedSVD y/o PCA y luego aplicar el K-means