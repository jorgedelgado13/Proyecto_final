...
num_cols_cluster = ["authorFollowers", "account_age_days", "mentions_count",
                    "hashtags_count", "content_length", "toxicity_score"]  # sin sentiment_polarity
...
for k in k_range:
    ...
    Xs = pipe_km.named_steps["scaler"].transform(dfc)      # silhouette en espacio escalado
...
```{python}
# --- Visualización básica y relación con target binario (0.5) ---
dfvis = dfc.copy()
...
# Scatter 2D con variables interpretables (sin sentiment_polarity)
...
Xs = best["model"].named_steps["scaler"].transform(dfc)
...
# PCA 2D para visualización (sin sentiment_polarity en las features)
...
toxbin = (dfc["toxicity_score"] >= 0.5).astype(int).values
